...
_server_data_url_ = 'https://downloads.guidos-skinbase.de/skin_topics/exendet_neon/topics_data.txt'
...
def select_dialog(dic_array):
    ...
    call = xbmcgui.Dialog().select('[COLOR blue]SKIN TOPICS DOWNLOADER[/COLOR]',dic_array['name_array'])
...
def download_file(file_download_url,file_download_path):
    ...
    try:
        ...
        dp.create('[COLOR blue]TOPICS ZIP DOWNLOADER[/COLOR]', 'Download file !', 'Please wait ...')
        dp.update(0)
        xbmc.sleep(1000)
...
def extract_zip(zipfile,extract_path,zip_pwd = None):
    ...
    dp = xbmcgui.DialogProgress()
    dp.create('[COLOR blue]TOPICS ZIP EXTRACTOR[/COLOR]', 'Unpacking data !', 'Please wait ...')
